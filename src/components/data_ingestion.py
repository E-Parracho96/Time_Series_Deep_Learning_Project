...
def data_ingestion(file1_path, file2_path):
    ...
    try:
        ...
        if missing_columns:
            raise ValueError(
                f"The following essential columns are missing in the DataFrame: {', '.join(missing_columns)}")
        ...
        df2 = df2.iloc[0:35041,1:]  # The sun_position_PT file has 1 more day, so we have to filter it and also filter the time column
